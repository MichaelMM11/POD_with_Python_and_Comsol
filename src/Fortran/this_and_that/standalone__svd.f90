program main
    ...
    !
      ...
      integer, parameter :: data_file_base_max = 20
    
      character average_char
      logical average_normalization
      real ( kind = rk ) average_value
      ...
      real ( kind = rk ), allocatable, dimension ( : ) :: point_average
      integer  point_num
      real ( kind = rk ), allocatable, dimension ( : ) :: sval
      real ( kind = rk ), allocatable, dimension ( :, : ) :: table
      real ( kind = rk ) tol
    ...
    !
      ...
      do
        ...
        if ( data_file_base_max <= data_file_base_num ) then
          ...
          write ( *, '(a)' ) &
            '  (There is an internal limit called DATA_FILE_BASE_MAX.)'
          exit
        end if
    !
    ...
    !
        if ( data_file_base_num == 1 ) then
          ...
          call r8mat_header_read ( file_name, comp_num, node_num )
    ...
    !
      ...
      do i = 1, data_file_base_num
        ...
        do
          ...
          call file_name_inc_nowrap ( data_file )
    ...
    !   write ( *, * ) 'DEBUG, Looping through files, now on II = ', ii
        ...
        do
          ...
          call r8mat_data_read ( data_file, comp_num, node_num, table )
    ...
    !
    !  If the average was requested, it is written to file SVD_000.TXT
    !
      ...
      do j = 1, basis_num
        ...
        call file_name_inc_nowrap ( basis_file )
    ...
    !
    !    Input, real ( kind = rk ) S, the associated singular value.
    !
    !    Input, real ( kind = rk ) U(M,N), the data values.
    !
    ...
    subroutine file_name_inc_nowrap ( file_name )
    ...
    !
      ...
      if ( lens <= 0 ) then
        write ( *, '(a)' ) ' '
        write ( *, '(a)' ) 'FILE_NAME_INC_NOWRAP - Fatal error!'
    ...
    !
    !    Output, integer IERROR, an error flag, which is zero
    ...
    !
    !    Output, real ( kind = rk ) R, the value that was read from the string.
    !
    ...
    !
    !    Input, integer BASIS_NUM, the number of basis vectors
    !    to be extracted.  0 < BASIS_NUM <= N.
    !
    !    real ( kind = rk ) A(M,N): the matrix whose
    !    singular values are to be computed. 
    !
    !  Output:
    !
    !    real ( kind = rk ) A(M,N); the first BASIS_NUM left singular vectors.
    !
    !    Output, real ( kind = rk ) SVAL(BASIS_NUM), the first BASIS_NUM
    !    singular values.
    !
      implicit none
      ...
      character jobvt
      integer lwork
      real ( kind = rk ) s(min(m,n))
      real ( kind = rk ) sval(basis_num)
      real ( kind = rk ) u(1,1)
      real ( kind = rk ) vt(1,1)
      real ( kind = rk ) work(3*min(m,n)+max(max(m,n),5*min(m,n)))
    ...
    !
      if ( basis_num <= 0 ) then
        ...
        write ( *, '(a)' ) 'SINGULAR_VECTORS - Fatal error!'
        write ( *, '(a)' ) '  BASIS_NUM <= 0.'
        stop 1
      end if
